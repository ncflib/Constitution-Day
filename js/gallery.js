...
d3.json('/constitution/js/facets.json').then(function(d){
...
d3.json('/constitution/js/lunr-index.json').then(function(d){
    monuments = d;
...
function drawGallery(data){
    ...
    let monument = galleryTiles.enter()
                    .append('a')
                    .attr('xlink:href', d => '/constitution/constitution/' + d.pid);

    monument.append('image')
        .style('opacity',0)
        .attr('xlink:href', d => '/constitution/assets/constitution/thumbs/' + d.source )
